    ...
    d3.json(platesUrl).then(function (plate_data) {
      ...
      for (let i = 0; i < data.features.length; i++) {
        ...
        if (location) {
          ...
          // Create Marker
          let circle_marker = L.circleMarker([latitude, longitude], {
            fillOpacity: 0.75,
            color: "white",
            fillColor: getColor(depth),
            radius: getRadius(mag)
          }).bindPopup(`<h5>${feature.properties.place}</h5><hr><h7>Depth: ${depth}m</h7>`);
      ...
      let markerLayer = L.layerGroup(circle_markers);
